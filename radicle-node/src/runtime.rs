...
impl Runtime {
    ...
    fn receive_listener() -> Option<UnixListener> {
        ...
        match libsystemd::activation::receive_descriptors_with_names(true) {
            Ok(descriptors) => match descriptors.into_iter().find_map(|(fd, name)| (name == NAME).then_some(fd)) {
                ...
                Some(_) => log::debug!(target: "node", "Unexpected type of file descriptor from systemd."),
                None => log::debug!(target: "node", "No file descriptor named '{NAME}' from systemd."),
            }
