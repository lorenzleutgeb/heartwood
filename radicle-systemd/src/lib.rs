...
use std::process::id;
use std::os::fd::RawFd;
use std::env::{var, remove_var, VarError};

const LISTEN_PID: &'static str = "LISTEN_PID";
const LISTEN_FDS: &'static str = "LISTEN_FDS";
const LISTEN_FDNAMES: &'static str = "LISTEN_FDNAMES";
...
pub fn listen_fd(name: &str) -> Result<Option<RawFd>, VarError> {
    let fd = match var(LISTEN_PID) {
        ...
        _ if var(LISTEN_FDS)? != "1" || var(LISTEN_FDNAMES).ok() != Some(name.to_string()) => Ok(None),
        _ => Ok(Some(SD_LISTEN_FDS_START + 0))
    };
